...
parse_t parse_delim(char **ptr, char delim)
{
    ...
    if(*current != delim) return PARSE_ERR;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    SKIP_BLANK(current);
    if(!IS_REG(current)) return PARSE_ERR;

    /* find register */
    temp = find_register(current);
    if(temp == REG_ERR) return PARSE_ERR;
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    SKIP_BLANK(current);
    if(!IS_LETTER(current)) return PARSE_ERR;
    /* allocate name and copy to it */
    int len = 0;
    while(IS_LETTER(current+len)||IS_DIGIT(current+len)) len++;

    sym_name = (char *)malloc(len+1);
    int i;
    for(i = 0;i<len;i++) sym_name[i] = current[i];
    sym_name[len] = '\0';
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    char *endptr;
    SKIP_BLANK(current);
    if(!IS_DIGIT(current)) return PARSE_ERR;

    int scale = IS_HEX(current) ? 16 : 10;

    /* calculate the digit, (NOTE: see strtoll()) */
    long val = strtoll(current, &endptr, scale);
    current = endptr;
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
	...
	*value = val;
	...
	int i;
	for(i = 0;i<len;i++) new[i] = current[i];
	new[len] = '\0';
	current+=len;
...
parse_t parse_mem(char **ptr, long *value, regid_t *regid)
{
	...
	int scale = IS_HEX(current) ? 16 : 10;
	char *endptr;
	val = strtoll(current, &endptr, scale);
	current = endptr;
	SKIP_BLANK(current);
	if(IS_END(current)) return PARSE_ERR;
    }
    
    if(*current != '(') return PARSE_ERR;
    current+=1;
    if(!IS_REG(current)) return PARSE_ERR;
    regid_t temp = find_register(current);
    if(temp == REG_ERR) return PARSE_ERR;
    current+=4;
    if(*current != ')') return PARSE_ERR;
    current+=1;
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    int len=0;
    while(IS_LETTER(current+len)||IS_DIGIT(current+len)) len++;
    if(current[len] != ':') return PARSE_ERR;
...
cont:
	...
	long *bin_code = (long *)&y86bin->codes[2];
	*bin_code = value;
	...
	*bin_code = value;
	...
	goto cont;
      ...
      case I_DIRECTIVE: {
	    ...
	    if(ret == PARSE_SYMBOL) add_reloc(name,y86bin);
	    

	    long *bin_code = (long *)y86bin->codes;
	    *bin_code = value;
	    ...
	    long diff = vmaddr % value;
	    if(diff!=0) vmaddr += value - diff;
	    y86bin->addr = vmaddr;
...
int relocate(void)
{
	...
	long *reloc_add;
	byte_t ins_type = HIGH(rtmp->y86bin->codes[0]);
	if(ins_type == I_IRMOVL) reloc_add = (long*)&rtmp->y86bin->codes[2];
	else if(ins_type == I_CALL || ins_type == I_JMP) reloc_add = (long *)&rtmp->y86bin->codes[1];
	else reloc_add = (long *)&rtmp->y86bin->codes[0];
...
int binfile(FILE *out)
{
    ...
    line_t *cur_loc = y86bin_listhead;
    int position = 0;

    /* binary write y86 code to output file (NOTE: see fwrite()) */
    while(cur_loc){
	if(position < cur_loc->y86bin.addr && cur_loc->y86bin.bytes != 0){
	    int differ = cur_loc->y86bin.addr - position;
	    int i;
	    int content = 0;
	    for(i=0;i<differ;i++) fwrite(&content,sizeof(byte_t),1,out);
	    position = cur_loc->y86bin.addr;
	}
	size_t count = fwrite(cur_loc->y86bin.codes, sizeof(byte_t), cur_loc->y86bin.bytes, out);
	if(count != cur_loc->y86bin.bytes) return -1;

	position += cur_loc->y86bin.bytes;
