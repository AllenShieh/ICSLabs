...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
	...
	if (val == 0) zero = TRUE;
	if (val < 0) sign = TRUE;
	if ( ((argA <= 0)==(argB < 0)) && ((argA <= 0)!=(val < 0)) ) ovf = TRUE;
	break;
    }
    case 1:
    {
...
stat_t nexti(y86sim_t *sim)
{
    ...
    /* actually not very useful, few steps are saved */
	...
	regid_t iregA = GET_REGA(code), iregB = GET_REGB(code);
	next_pc++;
	long_t val = get_reg_val(sim->r, iregA);
	if (cond_doit(sim->cc, ifun)==TRUE) set_reg_val(sim->r,iregB,val);
	sim->pc = next_pc;
	break;
      }
      case I_IRMOVL: /* 3:0 F:regB imm */{
	byte_t code1 = 0;
	if (!get_byte_val(sim->m, next_pc, &code1)) {
	  err_print("PC =  0x%x, Invalid instruction address", sim->pc);
	  return STAT_ADR;
	}
	regid_t iregB = GET_REGB(code1);
	next_pc++;
	long_t code2 = 0;
	if (!get_long_val(sim->m, next_pc, &code2)) {
	  err_print("PC = 0x%x, Invalid instruction address", sim->pc);
	  return STAT_ADR;
	}
	long_t val = code2;
	...
	regid_t iregA = GET_REGA(code1), iregB = GET_REGB(code1);
	next_pc++;
	long_t code2 = 0;
	if(!get_long_val(sim->m, next_pc, &code2)){
	  err_print("PC = 0x%x, Invalid instruction address", sim->pc);
	...
	long_t val = code2;
	next_pc+=4;
	long_t Aval = get_reg_val(sim->r, iregA);
	long_t Bval = get_reg_val(sim->r, iregB);
	set_long_val(sim->m, Bval+val, Aval);
	sim->pc = next_pc;
	break;
      }
	...
	long_t Bval = get_reg_val(sim->r, iregB);
	long_t code3 = 0;
	if(!get_long_val(sim->m, Bval + val, &code3)){
	  err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, Bval+val);
	  return STAT_ADR;
	}
	set_reg_val(sim->r, iregA, code3);
	sim->pc = next_pc;
	break;
      }
	...
	long_t Bval = get_reg_val(sim->r, iregB);
	next_pc++;
	long_t val = compute_alu(ifun, Aval, Bval);
	/*if(cond_doit(sim->cc, ifun)==TRUE)*/ set_reg_val(sim->r, iregB, val);
	sim->cc = compute_cc(ifun, Aval, Bval, val);
	sim->pc = next_pc;
	break;
      }
      case I_JMP: /* 7:x imm */{
	long_t code = 0;
	if(!get_long_val(sim->m, next_pc, &code)){
	  err_print("PC = 0x%x, Invalid instruction address", sim->pc);
	  return STAT_ADR;
	}
	long_t val = code;
	next_pc+=4;
	if(cond_doit(sim->cc, ifun)==TRUE) next_pc = val;
	sim->pc = next_pc;
	break;
      }
      case I_CALL: /* 8:x imm */{
	...
	long_t val = code;
	next_pc+=4;
	long_t ESPval = get_reg_val(sim->r, 4);
	set_reg_val(sim->r, 4, ESPval-4);
	ESPval = get_reg_val(sim->r, 4);
	set_long_val(sim->m, ESPval, next_pc);
	if(!get_long_val(sim->m, ESPval, &code)){
	  err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, ESPval);
	  return STAT_ADR;
	}
	sim->pc = val;
	break;
